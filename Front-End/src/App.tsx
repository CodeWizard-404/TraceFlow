...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children, requiredPermissions }) => {
    ...
    if (!user || !token) { return <Navigate to="/login" replace state={{ from: window.location.pathname }} />; }
    if (!permissionsLoaded) { return <div>Loading permissions...</div>;}
    const hasPermission = requiredPermissions.some(perm => effectivePermissions?.some(p => p.name === perm));
    return hasPermission ? children : <Navigate to="/access-denied" replace />;
};

// Role-based ProtectedRoute component
interface RoleProtectedRouteProps { children: JSX.Element; requiredRoles: string[]; }
const RoleProtectedRoute: React.FC<RoleProtectedRouteProps> = ({ children, requiredRoles }) => {
    ...
    if (!user || !token) { return <Navigate to="/login" replace state={{ from: window.location.pathname }} />; }
    if (!permissionsLoaded || !userRoles) {return <div>Loading permissions...</div>; }
        ...
        <div className="app-container">
            ...
            <main>
                ...
                <Routes>
                    ...
                    <Route path="/admin" element={<RoleProtectedRoute requiredRoles={[ROLES.ADMIN, ROLES.SUPER_ADMIN]}><AdminDashboard /></RoleProtectedRoute>} />
                    <Route path="/timesheet" element={<ProtectedRoute requiredPermissions={[PERMISSIONS.ACCESS_SUPERVISOR_TIMESHEETS]}><Timesheets /></ProtectedRoute>} />
                    <Route path="/timesheet-form" element={<ProtectedRoute requiredPermissions={[PERMISSIONS.CREATE_TIMESHEETS]}><TimesheetForm /></ProtectedRoute>} />
                    <Route path="/qr-scan" element={<ProtectedRoute requiredPermissions={[PERMISSIONS.SCAN_VISITS]}><QRScan /></ProtectedRoute>} />
                    <Route path="/visit/:idVisit" element={<ProtectedRoute requiredPermissions={[PERMISSIONS.ACCESS_VISIT_DETAILS]}><VisitDetails /></ProtectedRoute>} />
                    <Route path="/visit/:idVisit/validate-checklist" element={<ProtectedRoute requiredPermissions={[PERMISSIONS.LOG_VISITS]}><VisitValidation /></ProtectedRoute>} />
                    <Route path="/receipt-books" element={<ProtectedRoute requiredPermissions={[PERMISSIONS.ACCESS_RECEIPT_BOOKS]}><ReceiptBooks /></ProtectedRoute>} />
                    <Route path="/receipt-book/:bookID/history" element={<ProtectedRoute requiredPermissions={[PERMISSIONS.ACCESS_RECEIPT_BOOK_HISTORY]}><ReceiptBookHistory /></ProtectedRoute>} />
                    <Route path="/transfer-receipt-books" element={<ProtectedRoute requiredPermissions={[PERMISSIONS.TRANSFER_RECEIPT_BOOKS]}><TransferReceiptBook /></ProtectedRoute>} />
