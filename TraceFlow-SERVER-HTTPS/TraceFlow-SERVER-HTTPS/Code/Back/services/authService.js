...
const ERROR_MESSAGES = {
    ...
    USER_NOT_FOUND: 'Account not found2.',
...
class AuthService {
    ...
    static async syncKeycloakUser(identifier, keycloakId) {
        console.log('Entering syncKeycloakUser function');
        try {
            console.log('Syncing Keycloak user:', identifier, keycloakId);
    
            let user = await User.findOne({ where: { email: identifier } });
            console.log('User found in database:', user);
    
            if (!user) {
                console.log('User not found in database, creating new user:', identifier);
                ...
                console.log('Updating user with Keycloak ID:', user.userID);
                await user.update({ keycloakId });
                console.log('User updated with Keycloak ID:', user);
            } else if (user.keycloakId !== keycloakId) {
                console.error('Keycloak ID mismatch for user:', user.userID);
            ...
            console.error('Sync Keycloak user error:', error);
...
try {
    ...
    // Log the Keycloak user response data
    console.log('Keycloak User Response Data:', keycloakUserResponse.data);

    if (!keycloakUserResponse.data.length) {
        throw Object.assign(new Error(ERROR_MESSAGES.KEYCLOAK_USER_NOT_FOUND), { status: 404 });
    }
} catch (error) {
    console.error('Keycloak user error:', error);
