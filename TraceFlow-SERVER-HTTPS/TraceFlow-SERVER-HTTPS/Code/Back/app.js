...
app.use((req, res, next) => {
    ...
    req.on('end', () => {
        console.log('Raw body:', data || 'No body received');
        req.rawBody = data || 'No body';
    });
    req.on('error', err => {
...
// Early Redis initialization to prevent 'Redis client not initialized' errors
const { initializeRedis } = require('./config/redis');
const logger = require('./utils/logger');
async function ensureRedisInitialized() {
    try {
        await initializeRedis(); // Initializes both redisClient and redisSubClient
...
// Execute Redis initialization before loading other dependencies
ensureRedisInitialized().then(() => {
    ...
    // Set up API documentation with express-jsdoc-swagger
    ...
    // Configure ReDoc for API documentation UI
    ...
    async function startApp() {
        ...
        // Log super admin credentials in development mode
    ...
    logger.error('Redux initialization failed', {
        error: error.message,
        stack: error.stack,
        service: 'redis',
    });
