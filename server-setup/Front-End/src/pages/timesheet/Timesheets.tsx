...
const Timesheets: React.FC = () => {
  ...
  const { user, token, userRoles, effectivePermissions, permissionsLoaded } =
    useAuth();
    ...
    return {
      ...
      visits: matchingTimesheets.flatMap((ts) =>
        (ts.Visits || []).map((visit) => ({
          ...
          supervisorID: ts.supervisorID,
          date: new Date(
            new Date(visit.date).getTime() - 24 * 60 * 60 * 1000
          ).toISOString(),
        }))
      ),
      status: matchingTimesheets[0]?.status || "Not Scheduled",
                ...
                {weeks.map((week) => (
                  ...
                  >
                    ...
                    <span className="week-range">
                      ...
                      -{" "}
                      {week.days[4].toLocaleDateString("en-GB", {
                        day: "numeric",
                        month: "short",
                      })}{" "}
                      /
                  ...
                  <div className="week-details-header">
                    <div className="week-details-info">
                      <p className="week-range">
                        ...
                        -{" "}
                        {weekData.days[4].toLocaleDateString("en-GB", {
                          day: "numeric",
                          month: "short",
                        })}
                      </p>
