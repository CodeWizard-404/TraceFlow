...
import {
  getAllRegions, getAllGovernorates, getAllDelegations, getGeocode, getDirections, searchPlaces, getDistanceMatrix,
...
interface DirectionsResponse {
  routes: Array<{
    legs: Array<{
      ...
      end_address: string;
      steps: Array<{ polyline: { points: string }; instructions: string }>;
    }>;
  }>;
}
...
const defaultCenter = { lat: 36.8065, lng: 10.1815 }; // Center of Tunisia
...
const MapComponent: React.FC = () => {
  ...
  const routePath = useMemo(() => {
    if (!route?.routes?.[0]) return [];
    const points = route.routes[0].legs.flatMap(leg =>
      leg.steps.flatMap(step => polyline.decode(step.polyline.points).map(([lat, lng]) => ({ lat, lng })))
    );
    return points;
  }, [route]);
  ...
  // Handle map click for adding agent
    ...
    if (lat && lng) {
      try {
        ...
        if (!geocode.formatted_address) {
          toast.error('Unable to determine address for the selected location');
          return;
        }
  ...
  const handleEditAgent = useCallback(async () => {
    ...
    try {
      ...
      const updated = await updateAgent(editAgent.agentID, {
        ...editAgent,
  ...
  const handleMarkerDragEnd = useCallback(async (event: google.maps.MapMouseEvent, markerId: string) => {
    const lat = event.latLng?.lat();
    const lng = event.latLng?.lng();
    if (!lat || !lng || !draggingMarker) return;
    ...
    try {
      ...
      if (!geocode.formatted_address) {
        ...
        setAllMarkers(prev =>
          prev.map(marker =>
            marker.id === markerId ? { ...marker, ...draggingMarker.original } : marker
          )
        );
        setFilteredMarkers(prev =>
          prev.map(marker =>
  ...
  // Get directions from user location to marker
  ...
  const handleCalculateRoute = useCallback(async () => {
    ...
    try {
      ...
      const distanceMatrix = await getDistanceMatrix([origin], [destination], routeMode);
      setRoute(directions);
      setWaypoints([]);
      toast.success(`Distance: ${distanceMatrix[0].elements[0].distance.text}, Duration: ${distanceMatrix[0].elements[0].duration.text}`);
      // Center map on route
  ...
  // Memoized marker list component
  ...
  const ControlPanel = React.memo(({ onAddAgent, onReturnToLocation }: {
    ...
    <div className="control-panel">
      ...
      <div className="route-section">
        ...
        />
        <button
          onClick={() => userLocation && setOrigin(`${userLocation.lat},${userLocation.lng}`)}
          className="route-btn"
        >
          My Location
        </button>
        ...
        <input
          ...
          onKeyPress={(e) => {
            if (e.key === 'Enter') {
              setWaypoints([...waypoints, (e.target as HTMLInputElement).value]);
        ...
        <select
          value={routeMode}
          onChange={(e) => setRouteMode(e.target.value as 'driving' | 'walking')}
  ...
  return (
    <div className="map-container">
      ...
      <ControlPanel
        onAddAgent={() => setAddingAgentMode(true)}
        onReturnToLocation={handleReturnToCurrentLocation}
      />
      <MarkerList
            ...
            <Autocomplete
              ...
              onPlaceChanged={() => {
                if (autocomplete) {
                  ...
                  setNewAgent({ ...newAgent, address: place.formatted_address || '' });
                }
              }}
          ...
          <div className="modal-content">
            ...
            <Autocomplete
              ...
              onPlaceChanged={() => {
                if (autocomplete) {
                  const place = autocomplete.getPlace();
                  setEditAgent({ ...editAgent, location: place.formatted_address || '' });
                }
              }}
              options={{ componentRestrictions: { country: 'tn' } }}
            >
