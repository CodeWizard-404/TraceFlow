...
class RoleService {
    ...
    static async createRole(name, description, actorID) {
        try {
            ...
            // Cache role in Redis
            await RedisUtils.storeRole(role.roleID, {
                name: role.name,
                description: role.description,
                permissions: []
            });

            // Publish role creation event to Redis
            await RedisUtils.publishEvent('role:created', {
                roleID: role.roleID,
                name,
                description,
                actorID
    ...
    static async getAllRoles() {
        try {
            ...
            const cachedRoles = await RedisUtils.getAllRoles();
    ...
    static async getRoleById(roleID) {
        try {
            // Try to fetch from Redis cache
            const cachedRole = await RedisUtils.getRole(roleID);
            if (cachedRole) {
                return cachedRole;
            }
            ...
            // Cache role in Redis
            await RedisUtils.storeRole(roleID, {
                name: role.name,
                description: role.description,
                permissions: role.Permissions.map(p => ({
                    permissionID: p.permissionID,
    ...
    static async updateRole(roleID, updates, actorID) {
        try {
            ...
            await RedisUtils.storeRole(roleID, {
                name: role.name,
                description: role.description,
                permissions: (await role.getPermissions()).map(p => ({
                    permissionID: p.permissionID,
                    name: p.name,
                    description: p.description
                }))
            ...
            await RedisUtils.publishEvent('role:updated', {
                roleID,
                name: role.name,
                description: role.description,
                actorID
    ...
    static async deleteRole(roleID, actorID) {
        try {
            ...
            await RedisUtils.deleteRole(roleID);

            // Publish role deletion event to Redis
            await RedisUtils.publishEvent('role:deleted', {
                roleID,
                name: role.name,
                actorID
            });
    ...
    static async assignRolesToUser(userID, roleIDs, actorID) {
        try {
            ...
            if (newRoles.length > 0) {
                ...
                await RedisUtils.storeUser(userID, {
                    email: user.email,
                    roles: (await user.getRoles()).map(r => r.name)
                });

                // Publish role assignment event to Redis
                await RedisUtils.publishEvent('role:assigned', {
                    userID,
    ...
    static async revokeRolesFromUser(userID, roleIDs, actorID) {
        try {
            ...
            for (const roleID of roleIDs) {
                ...
                // Publish role revocation event to Redis
                await RedisUtils.publishEvent('role:revoked', {
                    userID,
                    roleID: role.roleID,
                    actorID
                });
    ...
    static async getRolesByUser(userID) {
        try {
            // Try to fetch from Redis cache
            const cachedUser = await RedisUtils.getUser(userID);
            if (cachedUser && cachedUser.roles && cachedUser.roles.length > 0) {
                const roles = await Role.findAll({
                    where: { name: cachedUser.roles },
                    include: [
                        {
                            model: Permission,
                            through: { attributes: [] },
    ...
    static async resetMainRolesToDefault(actorID) {
        try {
            ...
            // Default roles configuration (unchanged for brevity)
            const defaultRoles = [
                {
                    name: 'Super Admin',
            ...
            for (const defaultRole of defaultRoles) {
                ...
                const permissionsToRevoke = currentPermissions
                    ...
                    .map((p) => p.permissionID)
                    .filter((id) => allPermissions.some((perm) => perm.permissionID === id));
            ...
            // Publish role reset event to Redis
            await RedisUtils.publishEvent('role:reset', {
                actorID,
