...
class _EditVisitScreenState extends State<EditVisitScreen> {
  ...
  Future<void> _fetchAgentByPhone(String phone) async {
    ...
    try {
      ...
      if (newAgent != null && newAgent.agentID != null) {
        ...
        if (supervisorAgents.any((a) => a.agentID == newAgent.agentID)) {
          setState(() {
            _selectedAgentId = newAgent.agentID;
            _selectedDelegationId = newAgent.delegationID;
          });
          final locationDetails = await LocationService.getLocationDetailsById(
            newAgent.delegationID,
          );
          if (locationDetails['success'] &&
              locationDetails['addressInfo'] != null) {
            setState(() {
              _selectedRegionId = locationDetails['addressInfo']['regionID'];
              _selectedGovernorateId =
                  locationDetails['addressInfo']['governorateID'];
            });
          }
        } else {
          _showSnackBar('Agent not assigned to supervisor');
        }
      } else {
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      if (_selectedRegionId != null &&
        ...
        location =
            '${region['name']}, ${governorate['name']}, ${delegation['name']}';
      } else {
  ...
  Future<void> _showAgentDialog(
    ...
    final delegationAgents = List<Agent>.from(agentProvider.agents);
                    ...
                    child: Column(
                      ...
                      children: [
                        TextField(
                          ...
                          onChanged: (value) {
                            setDialogState(() {
                              selectedChecklists.retainWhere(
                                (c) => c.item.toLowerCase().contains(
                                  value.toLowerCase(),
                                ),
                              );
                            });
                    ...
                    child: Column(
                      ...
                      children: [
                        TextField(
                          ...
                          onChanged: (value) {
                            setDialogState(() {
                              selectedReasons.retainWhere(
                                (r) => r.item.toLowerCase().contains(
                                  value.toLowerCase(),
                                ),
                              );
                            });
