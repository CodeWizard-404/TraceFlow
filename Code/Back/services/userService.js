...
class UserService {
    ...
    static async assignGovernorateToUser(userID, governorateID, actorID) {
        if (!userID || !governorateID) {
            logger.warn(`Assign governorate failed: Missing fields, user: ${actorID}`);
            throw new Error(ERROR_MESSAGES.MISSING_FIELDS);
        }
        ...
        try {
            ...
            if (!user) {
                logger.warn(`Assign governorate failed: User ${userID} not found, user: ${actorID}`);
                throw new Error(ERROR_MESSAGES.USER_NOT_FOUND);
            }
            if (!user.Roles.some(role => role.name === process.env.ROLE_SUPERVISOR)) {
                logger.warn(`Assign governorate failed: Invalid role for user ${userID}, user: ${actorID}`);
                throw new Error(ERROR_MESSAGES.INVALID_ROLE_ASSIGNMENT);
            }

            const governorate = await Governorate.findByPk(governorateID);
            if (!governorate) {
                logger.warn(`Assign governorate failed: Governorate ${governorateID} not found, user: ${actorID}`);
    ...
    static async assignDelegationToUser(userID, delegationID, actorID) {
        if (!userID || !delegationID) {
            logger.warn(`Assign delegation failed: Missing fields, user: ${actorID}`);
            throw new Error(ERROR_MESSAGES.MISSING_FIELDS);
        }
        ...
        try {
            ...
            if (!user) {
                logger.warn(`Assign delegation failed: User ${userID} not found, user: ${actorID}`);
                throw new Error(ERROR_MESSAGES.USER_NOT_FOUND);
            }
            if (!user.Roles.some(role => role.name === process.env.ROLE_SUPERVISOR)) {
                logger.warn(`Assign delegation failed: Invalid role for user ${userID}, user: ${actorID}`);
                throw new Error(ERROR_MESSAGES.INVALID_ROLE_ASSIGNMENT);
            }
            ...
            if (!delegation) {
                logger.warn(`Assign delegation failed: Delegation ${delegationID} not found, user: ${actorID}`);
                throw new Error(ERROR_MESSAGES.DELEGATION_NOT_FOUND);
            }

            // Validate that the delegation's governorate is assigned to the user
            if (!user.Governorates.some(g => g.governorateID === delegation.governorateID)) {
                logger.warn(`Assign delegation failed: Governorate not assigned to user ${userID}, user: ${actorID}`);
                throw new Error(ERROR_MESSAGES.REGION_NOT_ASSIGNED);
            }
