...
class RoleController {
        ...
        try {
            ...
            if (!roleID) {
                logger.error('Role ID is required', {
                    ...
                    ip: req.ip,
                    userId: req.user.userID,
                    metadata: { error: 'Role ID is required' }
                });
            ...
            logger.error(`Failed to retrieve role: ${error.message}`, {
                traceId: req.traceId,
                route: 'roles',
                service: 'api',
                status: 404,
    ...
    static async getRolesByUser(req, res) {
        try {
            ...
            logger.error(`Failed to retrieve user roles: ${error.message}`, {
                traceId: req.traceId,
                route: 'roles',
                service: 'api',
                status: 404,
        ...
        try {
            ...
            if (!name) {
                logger.error('Role name is required', {
                    ...
                    ip: req.ip,
                    userId: req.user.userID,
                    metadata: { error: 'Role name is required' }
                });
    ...
    static async assignRolesToUser(req, res) {
        try {
            ...
            if (!userID || !Array.isArray(roleIDs) || roleIDs.length === 0) {
                logger.error('User ID and non-empty role IDs array are required', {
                    traceId: req.traceId,
                    route: 'roles',
                    service: 'api',
                    status: 400,
                    ...
                    ip: req.ip,
                    userId: req.user.userID,
                    metadata: { error: 'User ID and non-empty role IDs array are required' }
                });
            ...
            logger.info(`Assigned roles to user ${userID}`, {
                traceId: req.traceId,
                route: 'roles',
                service: 'api',
                status: 200,
            ...
            logger.error(`Failed to assign roles: ${error.message}`, {
                traceId: req.traceId,
                route: 'roles',
                service: 'api',
                status: 400,
    ...
    static async revokeRolesFromUser(req, res) {
        try {
            ...
            logger.info(`Revoked roles from user ${userID}`, {
                traceId: req.traceId,
                route: 'roles',
                service: 'api',
                status: 200,
            ...
            logger.error(`Failed to revoke roles: ${error.message}`, {
                traceId: req.traceId,
                route: 'roles',
                service: 'api',
                status: 400,
    ...
    static async resetMainRoles(req, res) {
        try {
            ...
            logger.error(`Failed to reset roles: ${error.message}`, {
                traceId: req.traceId,
                route: 'roles',
                service: 'api',
                status: 500,
