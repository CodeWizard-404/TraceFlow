...
const UserView: React.FC<UserViewProps> = ({
  ...
  const { effectivePermissions: authEffectivePermissions, user: currentUser } =
    useAuth();
  ...
  const [hasUnsavedSupervisorChanges, setHasUnsavedSupervisorChanges] =
    useState(false);
  const [hasUnsavedOverrideChanges, setHasUnsavedOverrideChanges] =
    useState(false);
  ...
  const userPermissions = useMemo(
    () => ({
      ...
      canCreatePermissionOverrides: authEffectivePermissions?.some(
        (p) =>
          p.name ===
          import.meta.env.VITE_PERMISSIONS_CREATE_PERMISSION_OVERRIDES
      ),
      canRemovePermissionOverrides: authEffectivePermissions?.some(
        (p) =>
          p.name ===
          import.meta.env.VITE_PERMISSIONS_REMOVE_PERMISSION_OVERRIDES
      ),
    }),
  ...
  const handleToggleSupervisor = (supervisor: User) => {
    ...
    const hasSupervisor = tempSupervisors.some(
      (s) => s.userID === supervisor.userID
    );
    if (hasSupervisor) {
      setTempSupervisors(
        tempSupervisors.filter((s) => s.userID !== supervisor.userID)
      );
    } else {
    ...
    try {
      ...
      if (isManager) {
        const currentSupervisorIds =
          selectedUser.supervisors?.map((s) => s.userID) || [];
        const toAssign = supervisorIds.filter(
          (id) => !currentSupervisorIds.includes(id)
        );
        const toRevoke = currentSupervisorIds.filter(
          (id) => !supervisorIds.includes(id)
        );
      ...
      if (isSupervisor) {
        const currentManagerIds =
          selectedUser.managers?.map((m) => m.userID) || [];
        const toAssign = managerIds.filter(
          (id) => !currentManagerIds.includes(id)
        );
        const toRevoke = currentManagerIds.filter(
  ...
  const toggleOverridePopup = (permissionID: string) => {
    setActiveOverridePopup(
      activeOverridePopup === permissionID ? null : permissionID
    );
    setExpandedClasses(new Set());
  };
  ...
  const renderRolePopupContent = (roleID: string) => {
    ...
    return (
      <>
        ...
        ) : Object.keys(getCategorizedPermissionsForRole(rolePermissions))
            .length > 0 ? (
                            ...
                            <div key={supervisor.userID} className="list-item">
                              <label>
                                <input
                                  ...
                                  disabled={
                                    loading ||
                                    !userPermissions.canRevokeSupervisors
                                  }
                                />
                    ...
                    userPermissions.canReadManagers && (
                      <div className="assignment-list">
                        ...
                        <div className="pagination">
                          ...
                          <span>
                            Page {managerPage} of{" "}
                            {Math.ceil(managerUsers.length / ITEMS_PER_PAGE)}
                          </span>
                          <button
