...
/**
 * Controller for managing checklist operations.
 */
class ChecklistController {
    ...
    static async getAllChecklists(req, res) {
        try {
            ...
            logger.info(`Fetched all checklists by user ${req.user.userID}, IP: ${req.ip}`);
            return res.status(200).json(checklists);
        } catch (error) {
            logger.error(`Get all checklists error: ${error.message}, user: ${req.user.userID}, IP: ${req.ip}`);
    ...
    static async getChecklistByID(req, res) {
        try {
            ...
            if (!checklistID) {
                logger.warn(`Get checklist failed: Missing checklistID, user: ${req.user.userID}, IP: ${req.ip}`);
                return res.status(400).json({ error: 'Checklist ID is required' });
            }
            const checklist = await ChecklistService.getItemById(checklistID);
            logger.info(`Fetched checklist ${checklistID} by user ${req.user.userID}, IP: ${req.ip}`);
            return res.status(200).json(checklist);
        } catch (error) {
            logger.error(`Get checklist error: ${error.message}, user: ${req.user.userID}, IP: ${req.ip}`);
    ...
    static async getChecklistsByVisitID(req, res) {
        try {
            ...
            if (!visitID) {
                logger.warn(`Get checklists by visit failed: Missing visitID, user: ${req.user.userID}, IP: ${req.ip}`);
                return res.status(400).json({ error: 'Visit ID is required' });
            }
            const checklists = await ChecklistService.getChecklistsByVisitId(visitID);
            logger.info(`Fetched checklists for visit ${visitID} by user ${req.user.userID}, IP: ${req.ip}`);
            return res.status(200).json(checklists);
        } catch (error) {
            logger.error(`Get checklists by visit error: ${error.message}, user: ${req.user.userID}, IP: ${req.ip}`);
    ...
    static async createChecklist(req, res) {
        try {
            const { text } = req.body;
            if (!text) {
                logger.warn(`Create checklist failed: Missing text, user: ${req.user.userID}, IP: ${req.ip}`);
            ...
            // Notify creator and manager of new checklist
            await NotificationService.triggerNotification({
                event: 'checklist:created',
                data: { checklistID: checklist.checklistID, text },
                metadata: { createdBy: req.user.email },
            });
            logger.info(`Checklist ${checklist.checklistID} created by user ${req.user.userID}, IP: ${req.ip}`);
            return res.status(201).json(checklist);
        } catch (error) {
            logger.error(`Create checklist error: ${error.message}, user: ${req.user.userID}, IP: ${req.ip}`);
    ...
    static async updateChecklist(req, res) {
        try {
            ...
            const { text } = req.body;
            if (!checklistID || !text) {
                logger.warn(`Update checklist failed: Missing checklistID or text, user: ${req.user.userID}, IP: ${req.ip}`);
            ...
            // Notify creator and manager of checklist update
            await NotificationService.triggerNotification({
                event: 'checklist:updated',
                data: { checklistID, text },
                metadata: { updatedBy: req.user.email },
            });
            logger.info(`Checklist ${checklistID} updated by user ${req.user.userID}, IP: ${req.ip}`);
            return res.status(200).json(checklist);
        } catch (error) {
            logger.error(`Update checklist error: ${error.message}, user: ${req.user.userID}, IP: ${req.ip}`);
    ...
    static async deleteChecklist(req, res) {
        try {
            const { id: checklistID } = req.params;
            if (!checklistID) {
                logger.warn(`Delete checklist failed: Missing checklistID, user: ${req.user.userID}, IP: ${req.ip}`);
                return res.status(400).json({ error: 'Checklist ID is required' });
            }
            await ChecklistService.deleteItem(checklistID, req.user.userID);
            // Notify creator and manager of checklist deletion
            await NotificationService.triggerNotification({
                event: 'checklist:deleted',
                data: { checklistID },
                metadata: { deletedBy: req.user.email },
            });
            logger.info(`Checklist ${checklistID} deleted by user ${req.user.userID}, IP: ${req.ip}`);
            return res.status(204).send();
        } catch (error) {
            logger.error(`Delete checklist error: ${error.message}, user: ${req.user.userID}, IP: ${req.ip}`);
