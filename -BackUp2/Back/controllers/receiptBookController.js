...
class ReceiptBookController {
    static async createReceiptBook(req, res) {
        try {
            ...
            const responseBook = { ...receiptBook.toJSON(), qrCode: receiptBook.qrCode.toString('base64') };
    ...
    static async getAllReceiptBooks(req, res) {
        try {
            ...
            // Ensure all books are plain objects and encode qrCode
            const responseBooks = receiptBooks.map(book => ({
                ...book,
                qrCode: book.qrCode ? Buffer.from(book.qrCode).toString('base64') : book.qrCode,
            }));
            logger.info(`Fetched ${responseBooks.length} receipt books by user ${req.user.userID} in ${Date.now() - startTime}ms, IP: ${req.ip}`, { ip: req.ip });
        ...
        try {
            ...
            const responseBooks = Array.isArray(receiptBooks)
                ? receiptBooks.map(book => ({
                    ...book.toJSON(),
                    qrCode: book.qrCode.toString('base64'),
                }))
                : { ...receiptBooks.toJSON(), qrCode: receiptBooks.qrCode.toString('base64') };
